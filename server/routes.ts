...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/portfolio/:id/screenshot", isAuthenticated, async (req, res) => {
    try {
      ...
      const screenshotUrl = `https://api.screenshotone.com/take?url=${encodeURIComponent(item.url)}&access_key=${screenshotApiKey}&block_cookie_banners=true&full_page=false&viewport_width=1920&viewport_height=1080&format=png&ignore_host_errors=true&delay=3`;
